...
static void usage(const char *argv0) {
    fprintf(stderr,
            ...
            "          [--max-iters N] [--start-db X] [--end-db Y] [--step-db D] [--target-errors N] [--max-frames N] \\\n"
...
static int parse_args(int argc, char **argv, sweep_cfg_t *cfg) {
    *cfg = (sweep_cfg_t){
    ...
    if (!cfg->out_path || cfg->target_errors <= 0 || cfg->max_frames < 0 || cfg->step_db <= 0.0 || cfg->max_iters <= 0) {
        return -1;
    }
    return 0;
}
...
static int run_sweep(const sweep_cfg_t *cfg) {
    ...
    for (int point = 0; point < point_count; point++) {
        ...
        const double raw_ber = (double)raw_errs / ((double)pc.cw_bits * (double)frames_run);
        const double decoded_ber = (double)decoded_errs / ((double)pc.cw_bits * (double)frames_run);
        const double frame_success = (double)success_frames / (double)frames_run;
        const unsigned long long payload_bits = (unsigned long long)pc.cw_bits * frames_run;
